...
const setInitCoordinate = (e: MouseEvent) => {
  ...
  if (0 == e.button && 1 === prop.nowTool.event) {
    ...
    if (
      state.status &&
      "polyline" === prop.nowAttr.type &&
      prop.nowAttr.attr.points
    ) {
      const [mx = 0, my = 0] = getMousePos(drop.value, e),
        [tx = 0, ty = 0] = [
          mx - prop.nowAttr.attr.transform.x,
          my - prop.nowAttr.attr.transform.y,
        ];

      prop.nowAttr.attr.points = `${prop.nowAttr.attr.points} ${tx - 5},${
        ty - 5
      } `;
      state.points.x = tx - 5;
      state.points.y = ty - 5;
...
const onCanvasMousemove = (e: MouseEvent) => {
  ...
  if (1 === prop.nowAttr.event) {
    ...
    let [mx = 0, my = 0] = getMousePos(drop.value, e),
      [x = 0, y = 0] = [mx - state.x, my - state.y],
      [tx = 0, ty = 0] = [
        mx - prop.nowAttr.attr.transform.x,
        my - prop.nowAttr.attr.transform.y,
      ];
    ...
    switch (prop.nowAttr.type) {
      case "line":
        [prop.nowAttr.attr.style.x2, prop.nowAttr.attr.style.y2] = state.shift
          ? getQuadrant(x, y)
          : [x, y];
        break;
      ...
      case "polyline":
        ...
        prop.nowAttr.attr.points = state.shift
          ? `${state.points.path} ${rx - 5},${ry - 5} `
          : `${state.points.path}${tx - 5},${ty - 5} `;
        break;
      case "rect":
        [prop.nowAttr.attr.style.width, prop.nowAttr.attr.style.height] =
          state.shift ? [x, x] : [x, y];
        break;

      case "ellipse":
        [prop.nowAttr.attr.style.rx, prop.nowAttr.attr.style.ry] = state.shift
          ? [x, x]
          : [x, y];
        break;
...
onMounted(() => {
  ...
  // 重置画布标尺刻度
  watch(() => [canvas.width, canvas.height],
    (n1, n2) => {
      setTimeout(() => {
        const { top, left } = drop.value.getBoundingClientRect();
        [canvas.dropX, canvas.dropY] = [left, top];
        [canvas.lineX, canvas.lineY] = [draw.value!.offsetWidth / 2, draw.value!.offsetHeight / 2];
        oScale.reset();
        oScale.reset();
      });
    },
      ...
      >
        <svg
          ...
          :style="{
            ...
            transform: `scale(${canvas.zoom * 0.01})`,
            ...setup.backsetup,
          }"
          id="svg"
          :xmlns="NS.SVG"
        ...
        >
          ...
          <g
            v-for="(o, i) in svgData"
            :key="o.id"
            v-show="o.attr.show"
            class="node"
            @mousedown="onSvgMousedown($event, o, i)"
            @mouseup="onMouseup"
            :data-selected="o.id === prop.nowAttr.selected"
